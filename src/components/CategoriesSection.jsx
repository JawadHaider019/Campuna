...
export default function CategoriesSection({ onSelectCategory }) {
  const handleCategoryClick = (categoryName, slug) => {
        ...
        {CATEGORIES.map((cat, index) => {
          ...
          return (
            <motion.div
              ...
              className="bg-white border border-white/40 p-4 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 group flex flex-col items-center text-center min-w-[120px] sm:min-w-[140px] lg:min-w-0 flex-shrink-0 snap-center cursor-pointer"
              onClick={() => handleCategoryClick(cat.name, slug)}
            >
