...
export default function PartnersSection({ onPartnerClick }) {
    ...
    useEffect(() => {
        const updateConstraints = () => {
            ...
            let cardWidth = 360;
            if (w >= 1024) cardWidth = 550;
            else if (w >= 768) cardWidth = 500;
            else if (w >= 640) cardWidth = 370;

            const gap = 16;
            const step = cardWidth + gap;
            const totalWidth = PROVIDERS.length * step - gap;

            const padding = w >= 768 ? 96 : 32;
            const viewportWidth = Math.min(w - padding, 1320);

            setConstraints(Math.max(0, totalWidth - viewportWidth));
        };
    ...
    }, [isHovered, isDragging, loopTrigger]);
    ...
    const handleDragEnd = () => {
        ...
        const gap = 16;
        const step = cardWidth + gap;
        const loopWidth = step * PROVIDERS.length;
        ...
        <section id="campuna-spotlight" className="py-10 sm:py-16 bg-sand relative overflow-x-hidden">
            <div className="max-w-8xl mx-auto px-4 md:px-12">
                ...
                <div className="relative">
                    ...
                    <div className="relative overflow-x-hidden cursor-grab active:cursor-grabbing">
                        <motion.div
                            drag="x"
